...
if __name__ == "__main__":
    ...
    for n in range(0,len(daqs_discovered)):
        ...
        # assign the max number of counter channels the board can sustain
            for c in range(0,max_counter_channels):
                ul.c_config_scan(device_info.board_num,c,ENUMS.CounterMode.PERIOD.PULSEWIDTH,
                                 ENUMS.CounterDebounceTime.DEBOUNCE500ns,ENUMS.CounterDebounceMode.TRIGGER_AFTER_STABLE,
                                 ENUMS.CounterEdgeDetection.RISING_EDGE,counter_tick_exp,c)
                                # clear the counter? TODO: not sure if this is needed or does anythign NOTE: doesn't seem to do anything
    ...
    #TODO: in the future, you'll need to loop over number of boards as well
    ...
    try:
        while True:
            ...
            for counter_num in range(0,max_counter_channels):
                ...
                # determine the range of the tick, ie greater than 0:1, 1:10, 10:100, beyond 100?
                if pulse_width_us > 0:
                    if (pulse_width_us > 0) & (pulse_width_us < 10):
                        print('Counter ' + str(counter_num) + ' detected 1 us event' + ' of ' + str(pulse_width_us) + ' us')
                    if (pulse_width_us >= 10) & (pulse_width_us < 100):
                        print('Counter ' + str(counter_num) + ' detected 10 us event' + ' of ' + str(pulse_width_us) + ' us')
                    if (pulse_width_us >= 100):
                        # TODO: need a way to distinguish whether there is no signal or the event is jsut greater than 100 us
                        print('Counter ' + str(counter_num) + ' detected 100 us event' + ' of ' + str(pulse_width_us) + ' us')
                    # NOTE: this function doesn't appear to work as it seems like it should
                    ul.c_clear(device_info.board_num, counter_num)

                else:
                    print('Counter ' + str(counter_num) + ' no event detected')
